...
QUnit.test( "sQuery.propFix integrity test", function( assert ) {
	assert.expect( 1 );

	//  This must be maintained and equal sQuery.attrFix when appropriate
	...
	assert.deepEqual( props, sQuery.propFix, "sQuery.propFix passes integrity check" );
} );
QUnit.test('sQuery.attr()', function(assert) {
	var text1 = sQuery('#text1')[0];
	assert.equal(sQuery.attr(text1, 'type'), 'text', 'check for type attribute');
	sQuery.attr(text1, "value", "t" )
	assert.equal( sQuery.attr(text1, "value" ), "t", "Check setting the value attribute" );
})
QUnit.test( "attr(String)", function( assert ) {
	...
	assert.equal( sQuery( "#text1" ).attr( "type" ), "text", "Check for type attribute" );
	assert.equal( sQuery( "#radio1" ).attr( "type" ), "radio", "Check for type attribute" );
	assert.equal( sQuery( "#check1" ).attr( "type" ), "checkbox", "Check for type attribute" );
	assert.equal( sQuery( "#simon1" ).attr( "rel" ), "bookmark", "Check for rel attribute" );
	assert.equal( sQuery( "#google" ).attr( "title" ), "Google!", "Check for title attribute" );
	assert.equal( sQuery( "#mark" ).attr( "hreflang" ), "en", "Check for hreflang attribute" );
	assert.equal( sQuery( "#en" ).attr( "lang" ), "en", "Check for lang attribute" );
	assert.equal( sQuery( "#simon" ).attr( "class" ), "blog link", "Check for class attribute" );
	assert.equal( sQuery( "#name" ).attr( "name" ), "name", "Check for name attribute" );
	assert.equal( sQuery( "#text1" ).attr( "name" ), "action", "Check for name attribute" );
	assert.ok( sQuery( "#form" ).attr( "action" ).indexOf( "formaction" ) >= 0, "Check for action attribute" );
	assert.equal( sQuery( "#text1" ).attr( "value", "t" ).attr( "value" ), "t", "Check setting the value attribute" );
	assert.equal( sQuery( "#text1" ).attr( "value", "" ).attr( "value" ), "", "Check setting the value attribute to empty string" );
	assert.equal( sQuery( "<div value='t'></div>" ).attr( "value" ), "t", "Check setting custom attr named 'value' on a div" );
	assert.equal( sQuery( "#form" ).attr( "blah", "blah" ).attr( "blah" ), "blah", "Set non-existent attribute on a form" );
	assert.equal( sQuery( "#foo" ).attr( "height" ), undefined, "Non existent height attribute should return undefined" );

	// [7472] & [3113] (form contains an input with name="action" or name="id")
	extras = sQuery( "<input id='id' name='id' /><input id='name' name='name' /><input id='target' name='target' />" ).appendTo( "#testForm" );
	assert.equal( sQuery( "#form" ).attr( "action", "newformaction" ).attr( "action" ), "newformaction", "Check that action attribute was changed" );
	assert.equal( sQuery( "#testForm" ).attr( "target" ), undefined, "Retrieving target does not equal the input with name=target" );
	assert.equal( sQuery( "#testForm" ).attr( "target", "newTarget" ).attr( "target" ), "newTarget", "Set target successfully on a form" );
	assert.equal( sQuery( "#testForm" ).removeAttr( "id" ).attr( "id" ), undefined, "Retrieving id does not equal the input with name=id after id is removed [#7472]" );

	// Bug #3685 (form contains input with name="name")
	assert.equal( sQuery( "#testForm" ).attr( "name" ), undefined, "Retrieving name does not retrieve input with name=name" );
	extras.remove();

	assert.equal( sQuery( "#text1" ).attr( "maxlength" ), "30", "Check for maxlength attribute" );
	assert.equal( sQuery( "#text1" ).attr( "maxLength" ), "30", "Check for maxLength attribute" );
	assert.equal( sQuery( "#area1" ).attr( "maxLength" ), "30", "Check for maxLength attribute" );
	...
	assert.equal( sQuery( "#tAnchor5" ).attr( "href" ), "#5", "Check for non-absolute href (an anchor)" );
	sQuery( "<a id='tAnchor6' href='#5' />" ).appendTo( "#qunit-fixture" );
	assert.equal( sQuery( "#tAnchor5" ).prop( "href" ), sQuery( "#tAnchor6" ).prop( "href" ), "Check for absolute href prop on an anchor" );

	sQuery( "<script type='jquery/test' src='#5' id='scriptSrc'></script>" ).appendTo( "#qunit-fixture" );
	assert.equal( sQuery( "#tAnchor5" ).prop( "href" ), sQuery( "#scriptSrc" ).prop( "src" ), "Check for absolute src prop on a script" );

	// list attribute is readonly by default in browsers that support it
	sQuery( "#list-test" ).attr( "list", "datalist" );
	assert.equal( sQuery( "#list-test" ).attr( "list" ), "datalist", "Check setting list attribute" );
	...
	assert.equal( sQuery( option ).prop( "selected" ), true, "Make sure that a single option is selected, even when in an optgroup." );

	$img = sQuery( "<img style='display:none' width='215' height='53' src='data/1x1.jpg'/>" ).appendTo( "body" );
	...
	styleElem = sQuery( "<div/>" ).appendTo( "#qunit-fixture" ).css( {
	...
	$button = sQuery( "<button>text</button>" ).insertAfter( "#button" );
	...
	assert.equal( sQuery( "#table" ).attr( "test:attrib" ), undefined, "Retrieving a non-existent attribute on a table with a colon does not throw an error." );
	assert.equal( sQuery( "#table" ).attr( "test:attrib", "foobar" ).attr( "test:attrib" ), "foobar", "Setting an attribute on a table with a colon does not throw an error." );

	$form = sQuery( "<form class='something'></form>" ).appendTo( "#qunit-fixture" );
	assert.equal( $form.attr( "class" ), "something", "Retrieve the class attribute on a form." );

	$a = sQuery( "<a href='#' onclick='something()'>Click</a>" ).appendTo( "#qunit-fixture" );
	assert.equal( $a.attr( "onclick" ), "something()", "Retrieve ^on attribute without anonymous function wrapper." );

	assert.ok( sQuery( "<div/>" ).attr( "doesntexist" ) === undefined, "Make sure undefined is returned when no attribute is found." );
	assert.ok( sQuery( "<div/>" ).attr( "title" ) === undefined, "Make sure undefined is returned when no attribute is found." );
	assert.equal( sQuery( "<div/>" ).attr( "title", "something" ).attr( "title" ), "something", "Set the title attribute." );
	assert.ok( sQuery().attr( "doesntexist" ) === undefined, "Make sure undefined is returned when no element is there." );
	assert.equal( sQuery( "<div/>" ).attr( "value" ), undefined, "An unset value on a div returns undefined." );
	assert.strictEqual( sQuery( "<select><option value='property'></option></select>" ).attr( "value" ), undefined, "An unset value on a select returns undefined." );

	$form = sQuery( "#form" ).attr( "enctype", "multipart/form-data" );
...
QUnit.test( "attr(String) in XML Files", function( assert ) {
	...
	assert.equal( sQuery( "locations", xml ).attr( "class" ), "foo", "Check class attribute in XML document" );
	assert.equal( sQuery( "location", xml ).attr( "for" ), "bar", "Check for attribute in XML document" );
	assert.equal( sQuery( "location", xml ).attr( "checked" ), "different", "Check that hooks are not attached in XML document" );
} );
...
QUnit.test( "attr(String, Function)", function( assert ) {
	...
	assert.equal(
		sQuery( "#text1" ).attr( "value", function() {
			return this.id;
		} ).attr( "value" ),
		"text1",
		"Set value from id"
	...
	assert.equal(
		sQuery( "#text1" ).attr( "title", function( i ) {
			return i;
		} ).attr( "title" ),
		"0",
...
QUnit.test( "attr(String, Object)", function( assert ) {
	...
	sQuery( "#name" ).attr( "name", "something" );
	assert.equal( sQuery( "#name" ).attr( "name" ), "something", "Set name attribute" );
	sQuery( "#name" ).attr( "name", null );
	assert.equal( sQuery( "#name" ).attr( "name" ), undefined, "Remove name attribute" );
	...
	$input = sQuery( "<input type='checkbox'/>" ).attr( "checked", true );
	...
	$input = sQuery( "<option/>" ).attr( "selected", true );
	...
	$input = sQuery( "#text1" ).prop( "readOnly", true ).prop( "readOnly", false ).attr( "readonly", true );
	...
	$input = sQuery( "#check2" ).attr( "checked", true ).attr( "checked", false ).prop( "checked", true );
	...
	$input = sQuery( "#check2" ).attr( "checked", false ).attr( "checked", "checked" );
	assert.equal( $input.attr( "checked" ), "checked", "Set checked to 'checked' (verified by .attr)" );

	$radios = sQuery( "#checkedtest" ).find( "input[type='radio']" );
	...
	$input = sQuery( "#text1" ).attr( "readonly", false ).prop( "readOnly", true );
	...
	$input = sQuery( "#name" ).attr( "maxlength", "5" );
	...
	$details = sQuery( "<details open></details>" ).appendTo( "#qunit-fixture" );
	...
	sQuery( "#foo" ).attr( "contenteditable", true );
	assert.equal( sQuery( "#foo" ).attr( "contenteditable" ), "true", "Enumerated attributes are set properly" );
	...
	sQuery.each( [ commentNode, textNode, attributeNode ], function( i, elem ) {
		var $elem = sQuery( elem );
	...
	sQuery.each( [ window, document, obj, "#firstp" ], function( i, elem ) {
	...
	table = sQuery( "#table" ).append( "<tr><td>cell</td></tr><tr><td>cell</td><td>cell</td></tr><tr><td>cell</td><td>cell</td></tr>" );
	...
	assert.equal( sQuery( "#area1" ).attr( "value" ), undefined, "Value attribute is distinct from value property." );

	// for #1070
	sQuery( "#name" ).attr( "someAttr", "0" );
	assert.equal( sQuery( "#name" ).attr( "someAttr" ), "0", "Set attribute to a string of '0'" );
	sQuery( "#name" ).attr( "someAttr", 0 );
	assert.equal( sQuery( "#name" ).attr( "someAttr" ), "0", "Set attribute to the number 0" );
	sQuery( "#name" ).attr( "someAttr", 1 );
	assert.equal( sQuery( "#name" ).attr( "someAttr" ), "1", "Set attribute to the number 1" );
	...
	// Type
	type = sQuery( "#check2" ).attr( "type" );
	try {
		sQuery( "#check2" ).attr( "type", "hidden" );
	...
	thrown = true;
	try {
		sQuery( check ).attr( "type", "checkbox" );
	} catch ( e ) {
		thrown = false;
	}
	assert.ok( thrown, "Exception thrown when trying to change type property" );
	assert.equal( "checkbox", sQuery( check ).attr( "type" ), "Verify that you can change the type of an input element that isn't in the DOM" );
	...
	sQuery( "#name" ).attr( "maxlength", "5" ).removeAttr( "nonexisting" );
	assert.equal( typeof sQuery( "#name" ).attr( "maxlength", undefined ), "object", ".attr('attribute', undefined) is chainable (#5571)" );
	assert.equal( sQuery( "#name" ).attr( "maxlength", undefined ).attr( "maxlength" ), "5", ".attr('attribute', undefined) does not change value (#5571)" );
	assert.equal( sQuery( "#name" ).attr( "nonexisting", undefined ).attr( "nonexisting" ), undefined, ".attr('attribute', undefined) does not create attribute (#5571)" );
} );

QUnit.test( "attr(non-ASCII)", function( assert ) {
	assert.expect( 2 );

	var $div = sQuery( "<div Ω='omega' aØc='alpha'></div>" ).appendTo( "#qunit-fixture" );
...
QUnit.test( "attr - extending the boolean attrHandle", function( assert ) {
	assert.expect( 1 );
	var called = false,
		_handle = sQuery.expr.attrHandle.checked || $.noop;
	sQuery.expr.attrHandle.checked = function() {
		called = true;
		_handle.apply( this, arguments );
	};
	sQuery( "input" ).attr( "checked" );
	called = false;
	sQuery( "input" ).attr( "checked" );
...
QUnit.test( "attr(String, Object) - Loaded via XML document", function( assert ) {
	...
	var xml = createDashboardXML(),
		titles = [];
	sQuery( "tab", xml ).each( function() {
		titles.push( sQuery( this ).attr( "title" ) );
	} );
...
QUnit.test( "attr('tabindex')", function( assert ) {
	...
	assert.equal( sQuery( "#listWithTabIndex" ).attr( "tabindex" ), "5", "not natively tabbable, with tabindex set to 0" );
	assert.equal( sQuery( "#divWithNoTabIndex" ).attr( "tabindex" ), undefined, "not natively tabbable, no tabindex set" );

	// anchor with href
	assert.equal( sQuery( "#linkWithNoTabIndex" ).attr( "tabindex" ), undefined, "anchor with href, no tabindex set" );
	assert.equal( sQuery( "#linkWithTabIndex" ).attr( "tabindex" ), "2", "anchor with href, tabindex set to 2" );
	assert.equal( sQuery( "#linkWithNegativeTabIndex" ).attr( "tabindex" ), "-1", "anchor with href, tabindex set to -1" );

	// anchor without href
	assert.equal( sQuery( "#linkWithNoHrefWithNoTabIndex" ).attr( "tabindex" ), undefined, "anchor without href, no tabindex set" );
	assert.equal( sQuery( "#linkWithNoHrefWithTabIndex" ).attr( "tabindex" ), "1", "anchor without href, tabindex set to 2" );
	assert.equal( sQuery( "#linkWithNoHrefWithNegativeTabIndex" ).attr( "tabindex" ), "-1", "anchor without href, no tabindex set" );
} );

QUnit.test( "attr('tabindex', value)", function( assert ) {
	assert.expect( 9 );

	var element = sQuery( "#divWithNoTabIndex" );
	...
	element = sQuery( "#linkWithTabIndex" );
...
QUnit.test( "removeAttr(String)", function( assert ) {
	...
	assert.equal( sQuery( "#mark" ).removeAttr( "class" ).attr( "class" ), undefined, "remove class" );
	assert.equal( sQuery( "#form" ).removeAttr( "id" ).attr( "id" ), undefined, "Remove id" );
	assert.equal( sQuery( "#foo" ).attr( "style", "position:absolute;" ).removeAttr( "style" ).attr( "style" ), undefined, "Check removing style attribute" );
	assert.equal( sQuery( "#form" ).attr( "style", "position:absolute;" ).removeAttr( "style" ).attr( "style" ), undefined, "Check removing style attribute on a form" );
	assert.equal( sQuery( "<div style='position: absolute'></div>" ).appendTo( "#foo" ).removeAttr( "style" ).prop( "style" ).cssText, "", "Check removing style attribute (#9699 Webkit)" );
	assert.equal( sQuery( "#fx-test-group" ).attr( "height", "3px" ).removeAttr( "height" ).get( 0 ).style.height, "1px", "Removing height attribute has no effect on height set with style attribute" );

	sQuery( "#check1" ).removeAttr( "checked" ).prop( "checked", true ).removeAttr( "checked" );
	...
	sQuery( "#text1" ).prop( "readOnly", true ).removeAttr( "readonly" );
	assert.equal( document.getElementById( "text1" ).readOnly, false, "removeAttr sets boolean properties to false" );

	sQuery( "#option2c" ).removeAttr( "selected" );
	assert.equal( sQuery( "#option2d" ).attr( "selected" ), "selected", "Removing `selected` from an option that is not selected does not remove selected from the currently selected option (#10870)" );

	try {
		$first = sQuery( "#first" ).attr( "contenteditable", "true" ).removeAttr( "contenteditable" );
	...
	$first = sQuery( "<div Case='mixed'></div>" );
...
QUnit.test( "removeAttr(Multi String, variable space width)", function( assert ) {
	...
	var div = sQuery( "<div id='a' alt='b' title='c' rel='d'></div>" ),
		tests = {
			id: "a",
			alt: "b",
			title: "c",
	...
	sQuery.each( tests, function( key ) {
...
QUnit.test( "prop(String, Object)", function( assert ) {
	...
	assert.equal( sQuery( "#text1" ).prop( "value" ), "Test", "Check for value attribute" );
	assert.equal( sQuery( "#text1" ).prop( "value", "Test2" ).prop( "defaultValue" ), "Test", "Check for defaultValue attribute" );
	assert.equal( sQuery( "#select2" ).prop( "selectedIndex" ), 3, "Check for selectedIndex attribute" );
	assert.equal( sQuery( "#foo" ).prop( "nodeName" ).toUpperCase(), "DIV", "Check for nodeName attribute" );
	assert.equal( sQuery( "#foo" ).prop( "tagName" ).toUpperCase(), "DIV", "Check for tagName attribute" );
	assert.equal( sQuery( "<option/>" ).prop( "selected" ), false, "Check selected attribute on disconnected element." );

	assert.equal( sQuery( "#listWithTabIndex" ).prop( "tabindex" ), 5, "Check retrieving tabindex" );
	sQuery( "#text1" ).prop( "readonly", true );
	assert.equal( document.getElementById( "text1" ).readOnly, true, "Check setting readOnly property with 'readonly'" );
	assert.equal( sQuery( "#label-for" ).prop( "for" ), "action", "Check retrieving htmlFor" );
	sQuery( "#text1" ).prop( "class", "test" );
	assert.equal( document.getElementById( "text1" ).className, "test", "Check setting className with 'class'" );
	assert.equal( sQuery( "#text1" ).prop( "maxlength" ), 30, "Check retrieving maxLength" );
	sQuery( "#table" ).prop( "cellspacing", 1 );
	assert.equal( sQuery( "#table" ).prop( "cellSpacing" ), "1", "Check setting and retrieving cellSpacing" );
	sQuery( "#table" ).prop( "cellpadding", 1 );
	assert.equal( sQuery( "#table" ).prop( "cellPadding" ), "1", "Check setting and retrieving cellPadding" );
	sQuery( "#table" ).prop( "rowspan", 1 );
	assert.equal( sQuery( "#table" ).prop( "rowSpan" ), 1, "Check setting and retrieving rowSpan" );
	sQuery( "#table" ).prop( "colspan", 1 );
	assert.equal( sQuery( "#table" ).prop( "colSpan" ), 1, "Check setting and retrieving colSpan" );
	sQuery( "#table" ).prop( "usemap", 1 );
	assert.equal( sQuery( "#table" ).prop( "useMap" ), 1, "Check setting and retrieving useMap" );
	sQuery( "#table" ).prop( "frameborder", 1 );
	assert.equal( sQuery( "#table" ).prop( "frameBorder" ), 1, "Check setting and retrieving frameBorder" );
} );
...
QUnit.test( "prop(String, Object) on null/undefined", function( assert ) {
  ...
  assert.expect( 14 );
	...
	assert.equal( sQuery( option ).prop( "selected" ), true, "Make sure that a single option is selected, even when in an optgroup." );
	assert.equal( sQuery( document ).prop( "nodeName" ), "#document", "prop works correctly on document nodes (bug #7451)." );

	attributeNode = document.createAttribute( "irrelevant" );
	commentNode = document.createComment( "some comment" );
	textNode = document.createTextNode( "some text" );
	obj = {};
	sQuery.each( [ document, attributeNode, commentNode, textNode, obj, "#firstp" ], function( i, ele ) {
		assert.strictEqual( sQuery( ele ).prop( "nonexisting" ), undefined, "prop works correctly for non existing attributes (bug #7500)." );
	} );

	obj = {};
	sQuery.each( [ document, obj ], function( i, ele ) {
		var $ele = sQuery( ele );
		$ele.prop( "nonexisting", "foo" );
		assert.equal( $ele.prop( "nonexisting" ), "foo", "prop(name, value) works correctly for non existing attributes (bug #7500)." );
	} );
	sQuery( document ).removeProp( "nonexisting" );

	$form = sQuery( "#form" ).prop( "enctype", "multipart/form-data" );
	assert.equal( $form.prop( "enctype" ), "multipart/form-data", "Set the enctype of a form (encoding in IE6/7 #6743)" );
} );
...
QUnit.test( "prop('tabindex')", function( assert ) {
	...
	assert.equal( sQuery( "#inputWithoutTabIndex" ).prop( "tabindex" ), 0, "input without tabindex" );
	assert.equal( sQuery( "#buttonWithoutTabIndex" ).prop( "tabindex" ), 0, "button without tabindex" );
	assert.equal( sQuery( "#textareaWithoutTabIndex" ).prop( "tabindex" ), 0, "textarea without tabindex" );

	// elements not natively tabbable
	assert.equal( sQuery( "#listWithTabIndex" ).prop( "tabindex" ), 5, "not natively tabbable, with tabindex set to 0" );
	assert.equal( sQuery( "#divWithNoTabIndex" ).prop( "tabindex" ), -1, "not natively tabbable, no tabindex set" );

	// anchor with href
	assert.equal( sQuery( "#linkWithNoTabIndex" ).prop( "tabindex" ), 0, "anchor with href, no tabindex set" );
	assert.equal( sQuery( "#linkWithTabIndex" ).prop( "tabindex" ), 2, "anchor with href, tabindex set to 2" );
	assert.equal( sQuery( "#linkWithNegativeTabIndex" ).prop( "tabindex" ), -1, "anchor with href, tabindex set to -1" );

	// anchor without href
	assert.equal( sQuery( "#linkWithNoHrefWithNoTabIndex" ).prop( "tabindex" ), -1, "anchor without href, no tabindex set" );
	assert.equal( sQuery( "#linkWithNoHrefWithTabIndex" ).prop( "tabindex" ), 1, "anchor without href, tabindex set to 2" );
	assert.equal( sQuery( "#linkWithNoHrefWithNegativeTabIndex" ).prop( "tabindex" ), -1, "anchor without href, no tabindex set" );
} );

QUnit.test( "image.prop( 'tabIndex' )", function( assert ) {
	assert.expect( 1 );
	var image = sQuery("<img src='data/1x1.jpg' />")
		.appendTo("#qunit-fixture");
...
QUnit.test( "prop('tabindex', value)", function( assert ) {
	...
	var clone,
		element = sQuery( "#divWithNoTabIndex" );
...
QUnit.test( "option.prop('selected', true) affects select.selectedIndex (gh-2732)", function( assert ) {
	...
	function addOptions( $elem ) {
		return $elem.append(
			sQuery( "<option/>" ).val( "a" ).text( "One" ),
			sQuery( "<option/>" ).val( "b" ).text( "Two" ),
			sQuery( "<option/>" ).val( "c" ).text( "Three" )
		)
...
QUnit.test( "removeProp(String)", function( assert ) {
	...
	assert.strictEqual(
		sQuery( "#firstp" ).prop( "nonexisting", "foo" ).removeProp( "nonexisting" )[ 0 ][ "nonexisting" ],
		undefined,
	...
	sQuery.each( [ commentNode, textNode, attributeNode ], function( i, ele ) {
		var $ele = sQuery( ele );
		$ele.prop( "nonexisting", "foo" ).removeProp( "nonexisting" );
		assert.strictEqual( ele[ "nonexisting" ], undefined, "removeProp works correctly on non DOM element nodes (bug #7500)." );
	} );
...
QUnit.test( "val() after modification", function( assert ) {
	...
	assert.equal( sQuery( "#text1" ).val(), "bla", "Check for modified value of input element" );
} );

QUnit.test( "val()", function( assert ) {

	assert.expect( 20 + ( sQuery.fn.serialize ? 6 : 0 ) );

	var checks, $button;
	assert.equal( sQuery( "#text1" ).val(), "Test", "Check for value of input element" );

	// ticket #1714 this caused a JS error in IE
	assert.equal( sQuery( "#first" ).val(), "", "Check a paragraph element to see if it has a value" );
	assert.ok( sQuery( [] ).val() === undefined, "Check an empty sQuery object will return undefined from val" );

	assert.equal( sQuery( "#select2" ).val(), "3", "Call val() on a single='single' select" );

	assert.deepEqual( sQuery( "#select3" ).val(), [ "1", "2" ], "Call val() on a multiple='multiple' select" );

	assert.equal( sQuery( "#option3c" ).val(), "2", "Call val() on a option element with value" );

	assert.equal( sQuery( "#option3a" ).val(), "", "Call val() on a option element with empty value" );

	assert.equal( sQuery( "#option3e" ).val(), "no value", "Call val() on a option element with no value attribute" );

	assert.equal( sQuery( "#option3a" ).val(), "", "Call val() on a option element with no value attribute" );

	sQuery( "#select3" ).val( "" );
	assert.deepEqual( sQuery( "#select3" ).val(), [ "" ], "Call val() on a multiple='multiple' select" );
	...
	assert.equal( sQuery( "#select5" ).val(), "3", "Check value on ambiguous select." );

	sQuery( "#select5" ).val( 1 );
	assert.equal( sQuery( "#select5" ).val(), "1", "Check value on ambiguous select." );

	sQuery( "#select5" ).val( 3 );
	assert.equal( sQuery( "#select5" ).val(), "3", "Check value on ambiguous select." );

	assert.strictEqual(
		sQuery( "<select name='select12584' id='select12584'><option value='1' disabled='disabled'>1</option></select>" ).val(),
		null,
		"Select-one with only option disabled (#12584)"
	);

	if ( sQuery.fn.serialize ) {
		checks = sQuery( "<input type='checkbox' name='test' value='1'/><input type='checkbox' name='test' value='2'/><input type='checkbox' name='test' value=''/><input type='checkbox' name='test'/>" ).appendTo( "#form" );
	...
	$button = sQuery( "<button value='foobar'>text</button>" ).insertAfter( "#button" );
	...
	assert.equal( sQuery( "<option/>" ).val( "test" ).attr( "value" ), "test", "Setting value sets the value attribute" );
} );
...
QUnit.test( "val() with non-matching values on dropdown list", function( assert ) {
	...
	sQuery( "#select5" ).val( "" );
	assert.equal( sQuery( "#select5" ).val(), null, "Non-matching set on select-one" );

	var select6 = sQuery( "<select multiple id=\"select6\"><option value=\"1\">A</option><option value=\"2\">B</option></select>" ).appendTo( "#form" );
	sQuery( select6 ).val( "nothing" );
	assert.deepEqual( sQuery( select6 ).val(), [], "Non-matching set (single value) on select-multiple" );

	sQuery( select6 ).val( [ "nothing1", "nothing2" ] );
	assert.deepEqual( sQuery( select6 ).val(), [], "Non-matching set (array of values) on select-multiple" );
...
if ( "value" in document.createElement( "meter" ) &&
	...
	QUnit.test( "val() respects numbers without exception (Bug #9319)", function( assert ) {
		...
		var $meter = sQuery( "<meter min='0' max='10' value='5.6'></meter>" ),
			$progress = sQuery( "<progress max='10' value='1.5'></progress>" );
...
QUnit.test( "val(Array of Numbers) (Bug #7123)", function( assert ) {
	assert.expect( 4 );
	sQuery( "#form" ).append( "<input type='checkbox' name='arrayTest' value='1' /><input type='checkbox' name='arrayTest' value='2' /><input type='checkbox' name='arrayTest' value='3' checked='checked' /><input type='checkbox' name='arrayTest' value='4' />" );
	var elements = sQuery( "input[name=arrayTest]" ).val( [ 1, 2 ] );
...
QUnit.test( "val(Function) with incoming value", function( assert ) {
	...
	var oldVal = sQuery( "#text1" ).val();

	sQuery( "#text1" ).val( function( i, val ) {
	...
	oldVal = sQuery( "#select1" ).val();

	sQuery( "#select1" ).val( function( i, val ) {
		assert.equal( val, oldVal, "Make sure the incoming value is correct." );
		return "3";
	} );

	assert.equal( sQuery( "#select1" ).val(), "3", "Check for modified (via val(String)) value of select element" );
	...
	assert.equal( sQuery( "#select1" ).val(), "2", "Check for modified (via val(Number)) value of select element" );

	sQuery( "#select1" ).append( "<option value='4'>four</option>" );
	...
	assert.equal( sQuery( "#select1" ).val(), "4", "Should be possible to set the val() to a newly created option" );
} );
...
QUnit.test( "val(select) after form.reset() (Bug #2551)", function( assert ) {
	...
	sQuery( "<form id='kk' name='kk'><select id='kkk'><option value='cf'>cf</option><option value='gf'>gf</option></select></form>" ).appendTo( "#qunit-fixture" );

	sQuery( "#kkk" ).val( "gf" );

	document[ "kk" ].reset();

	assert.equal( sQuery( "#kkk" )[ 0 ].value, "cf", "Check value of select after form reset." );
	assert.equal( sQuery( "#kkk" ).val(), "cf", "Check value of select after form reset." );
...
var testRemoveClass = function( valueObj, assert ) {
	...
	var $set = sQuery( "#qunit-fixture div" ),
	...
	// using contents will get regular, text, and comment nodes
	$set = sQuery( "#nonnodes" ).contents();
	$set.removeClass( valueObj( "asdf" ) );
	assert.ok( !$set.hasClass( "asdf" ), "Check node,textnode,comment for removeClass" );

	sQuery( div ).removeClass( valueObj( "foo" ) );
	assert.strictEqual( sQuery( div ).attr( "class" ), undefined, "removeClass doesn't create a class attribute" );
	...
	sQuery( div ).removeClass( valueObj( "test" ) );
...
QUnit.test( "removeClass(Function) with incoming value", function( assert ) {
	...
	var $divs = sQuery( "#qunit-fixture div" ).addClass( "test" ), old = $divs.map( function() {
		return sQuery( this ).attr( "class" );
	} );
...
QUnit.test( "removeClass() removes duplicates", function( assert ) {
	...
	var $div = sQuery( sQuery.parseHTML( "<div class='x x x'></div>" ) );
...
QUnit.test( "removeClass(undefined) is a no-op", function( assert ) {
	...
	var $div = sQuery( "<div class='base second'></div>" );
...
var testToggleClass = function( valueObj, assert ) {
	...
	assert.ok( sQuery._data( e[ 0 ], "__className__" ) === "testD testE", "Assert data was stored" );
	e.toggleClass();
...
QUnit.test( "addClass, removeClass, hasClass on many elements", function( assert ) {
	...
	var elem = sQuery( "<p>p0</p><p>p1</p><p>p2</p>" );
	...
	assert.ok( sQuery( "<p class='hi'>p0</p><p>p1</p><p>p2</p>" ).hasClass( "hi" ),
		"Did find a class in the first element" );
	assert.ok( sQuery( "<p>p0</p><p class='hi'>p1</p><p>p2</p>" ).hasClass( "hi" ),
		"Did find a class in the second element" );
	assert.ok( sQuery( "<p>p0</p><p>p1</p><p class='hi'>p2</p>" ).hasClass( "hi" ),
		"Did find a class in the last element" );

	assert.ok( sQuery( "<p class='hi'>p0</p><p class='hi'>p1</p><p class='hi'>p2</p>" ).hasClass( "hi" ),
		"Did find a class when present in all elements" );

	assert.ok( !sQuery( "<p class='hi0'>p0</p><p class='hi1'>p1</p><p class='hi2'>p2</p>" ).hasClass( "hi" ),
...
QUnit.test( "contents().hasClass() returns correct values", function( assert ) {
	...
	var $div = sQuery( "<div><span class='foo'></span><!-- comment -->text</div>" ),
...
QUnit.test( "hasClass correctly interprets non-space separators (#13835)", function( assert ) {
	...
	var
		...
		classes = sQuery.map( map, function( separator, label ) {
			return " " + separator + label + separator + " ";
		} ),
		$div = sQuery( "<div class='" + classes + "'></div>" );

	sQuery.each( map, function( label ) {
...
QUnit.test( "SVG class manipulation (gh-2199)", function( assert ) {
	...
	sQuery.each( [
		...
		var elem = sQuery( createSVGElement( this ) );
