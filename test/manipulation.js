...
// Ensure that an extended Array prototype doesn't break sQuery
...
QUnit.test( "text()", function( assert ) {
    ...
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for merged text of more then one element." );

    // Check serialization of text values
    assert.equal( sQuery( document.createTextNode( "foo" ) ).text(), "foo", "Text node was retrieved from .text()." );
    assert.notEqual( sQuery( document ).text(), "", "Retrieving text for the document retrieves all text (#10724)." );

    // Retrieve from document fragments #10864
    frag = document.createDocumentFragment();
    frag.appendChild( document.createTextNode( "foo" ) );

    assert.equal( sQuery( frag ).text(), "foo", "Document Fragment Text node was retrieved from .text()." );

    // $newLineTest = sQuery( "<div>test<br/>testy</div>" ).appendTo( "#moretests" );
...
function testText( valueObj, assert ) {
    ...
    assert.equal( sQuery( "#foo" ).text( val )[ 0 ].innerHTML.replace( />/g, "&gt;" ), "&lt;div&gt;&lt;b&gt;Hello&lt;/b&gt; cruel world!&lt;/div&gt;", "Check escaped text" );

    // using contents will get comments regular, text, and comment nodes
    j = sQuery( "#nonnodes" ).contents();
    j.text( valueObj( "hi!" ) );
    assert.equal( sQuery( j[ 0 ] ).text(), "hi!", "Check node,textnode,comment with text()" );
    ...
    $multipleElements = sQuery( "<div>Hello</div>" ).add( "<div>World</div>" );
...
QUnit.test( "text(Function) with incoming value", function( assert ) {
    ...
    sQuery( "#sap" ).text( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return "foobar";
    } );

    assert.equal( sQuery( "#sap" ).text(), "foobar", "Check for merged text of more then one element." );
} );
...
function testAppendForObject( valueObj, isFragment, assert ) {
    ...
    if ( isFragment ) {
        ...
        sQuery( el ).contents().each( function() {
    ...
    assert.equal( $base.clone().append( valueObj( sQuery( "#yahoo, #first" ).clone() ) ).text(),
        text + "YahooTry them out:",
        "Check for appending of sQuery object" + type
    );
    ...
    assert.equal( $base.clone().append( valueObj( [ sQuery( "#first" ).clone(), sQuery( "#yahoo, #google" ).clone() ] ) ).text(),
        text + "Try them out:GoogleYahoo",
        "Check for appending of array of sQuery objects"
    );
...
function testAppend( valueObj, assert ) {
    ...
    result = sQuery( "#first" ).append( valueObj( "<b>buga</b>" ) );

    assert.equal( result.text(), defaultText + "buga", "Check if text appending works" );
    assert.equal( sQuery( "#select3" ).append( valueObj( "<option value='appendTest'>Append Test</option>" ) ).find( "option:last-child" ).attr( "value" ), "appendTest", "Appending html options to select element" );

    sQuery( "#qunit-fixture form" ).append( valueObj( "<input name='radiotest' type='radio' checked='checked' />" ) );
    sQuery( "#qunit-fixture form input[name=radiotest]" ).each( function() {
        assert.ok( sQuery( this ).is( ":checked" ), "Append checked radio" );
    } ).remove();

    sQuery( "#qunit-fixture form" ).append( valueObj( "<input name='radiotest2' type='radio' checked    =   'checked' />" ) );
    sQuery( "#qunit-fixture form input[name=radiotest2]" ).each( function() {
        assert.ok( sQuery( this ).is( ":checked" ), "Append alternately formated checked radio" );
    } ).remove();

    sQuery( "#qunit-fixture form" ).append( valueObj( "<input name='radiotest3' type='radio' checked />" ) );
    sQuery( "#qunit-fixture form input[name=radiotest3]" ).each( function() {
        assert.ok( sQuery( this ).is( ":checked" ), "Append HTML5-formated checked radio" );
    } ).remove();

    sQuery( "#qunit-fixture form" ).append( valueObj( "<input type='radio' checked='checked' name='radiotest4' />" ) );
    sQuery( "#qunit-fixture form input[name=radiotest4]" ).each( function() {
        assert.ok( sQuery( this ).is( ":checked" ), "Append with name attribute after checked attribute" );
    } ).remove();

    message = "Test for appending a DOM node to the contents of an iframe";
    iframe = sQuery( "#iframe" )[ 0 ];
    iframeDoc = iframe.contentDocument || iframe.contentWindow && iframe.contentWindow.document;

    try {
        if ( iframeDoc && iframeDoc.body ) {
            assert.equal( sQuery( iframeDoc.body ).append( valueObj( "<div id='success'>test</div>" ) )[ 0 ].lastChild.id, "success", message );
        } else {
    ...
    sQuery( "<fieldset/>" ).appendTo( "#form" ).append( valueObj( "<legend id='legend'>test</legend>" ) );
    assert.t( "Append legend", "#legend", [ "legend" ] );

    $map = sQuery( "<map/>" ).append( valueObj( "<area id='map01' shape='rect' coords='50,50,150,150' href='http://www.jquery.com/' alt='sQuery'>" ) );
    ...
    sQuery( "#select1" ).append( valueObj( "<OPTION>Test</OPTION>" ) );
    assert.equal( sQuery( "#select1 option:last-child" ).text(), "Test", "Appending OPTION (all caps)" );

    sQuery( "#select1" ).append( valueObj( "<optgroup label='optgroup'><option>optgroup</option></optgroup>" ) );
    assert.equal( sQuery( "#select1 optgroup" ).attr( "label" ), "optgroup", "Label attribute in newly inserted optgroup is correct" );
    assert.equal( sQuery( "#select1 option" ).last().text(), "optgroup", "Appending optgroup" );
    ...
    sQuery.each( "thead tbody tfoot colgroup caption tr th td".split( " " ), function( i, name ) {
        ...
        assert.ok( sQuery.parseHTML( "<" + name + "/>" ).length, name + " wrapped correctly" );
    } );

    sQuery( "#table colgroup" ).append( valueObj( "<col/>" ) );
    assert.equal( sQuery( "#table colgroup col" ).length, 1, "Append col" );
    ...
    assert.equal( "Two nodes", sQuery( "<div />" ).append( "Two", " nodes" ).text(), "Appending two text nodes (#4011)" );
    assert.equal( sQuery( "<div />" ).append( "1", "", 3 ).text(), "13", "If median is false-like value, subsequent arguments should not be ignored" );

    // using contents will get comments regular, text, and comment nodes
    j = sQuery( "#nonnodes" ).contents();
    d = sQuery( "<div/>" ).appendTo( "#nonnodes" ).append( j );

    assert.equal( sQuery( "#nonnodes" ).length, 1, "Check node,textnode,comment append moved leaving just the div" );
    assert.equal( d.contents().length, 3, "Check node,textnode,comment append works" );
    d.contents().appendTo( "#nonnodes" );
    d.remove();
    assert.equal( sQuery( "#nonnodes" ).contents().length, 3, "Check node,textnode,comment append cleanup worked" );

    $input = sQuery( "<input type='checkbox'/>" ).prop( "checked", true ).appendTo( "#testForm" );
    assert.equal( $input[ 0 ].checked, true, "A checked checkbox that is appended stays checked" );

    $radioChecked = sQuery( "input[type='radio'][name='R1']" ).eq( 1 );
    $radioParent = $radioChecked.parent();
    $radioUnchecked = sQuery( "<input type='radio' name='R1' checked='checked'/>" ).appendTo( $radioParent );
    $radioChecked.trigger( "click" );
    $radioUnchecked[ 0 ].checked = false;

    sQuery( "<div/>" ).insertBefore( $radioParent ).append( $radioParent );
    ...
    assert.equal( sQuery( "<div/>" ).append( valueObj( "option<area/>" ) )[ 0 ].childNodes.length, 2, "HTML-string with leading text should be processed correctly" );
}

QUnit.test( "append(String|Element|Array<Element>|sQuery)", function( assert ) {
...
QUnit.test( "append(Function) returns String", function( assert ) {
    ...
    defaultText = "Try them out:";
    old = sQuery( "#first" ).html();

    result = sQuery( "#first" ).append( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return "<b>buga</b>";
    } );
    assert.equal( result.text(), defaultText + "buga", "Check if text appending works" );

    select = sQuery( "#select3" );
    old = select.html();
...
QUnit.test( "append(Function) returns Element", function( assert ) {
    ...
    sQuery( "#sap" ).append( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return document.getElementById( "first" );
    } );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for appending of element" );
} );
...
QUnit.test( "append(Function) returns Array<Element>", function( assert ) {
    ...
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for appending of array of elements" );
} );

QUnit.test( "append(Function) returns sQuery", function( assert ) {
    ...
    sQuery( "#sap" ).append( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return sQuery( "#yahoo, #first" );
    } );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for appending of sQuery object" );
} );

QUnit.test( "append(Function) returns Number", function( assert ) {

    assert.expect( 2 );
    var old = sQuery( "#sap" ).html();

    sQuery( "#sap" ).append( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return 5;
    } );
    assert.ok( sQuery( "#sap" )[ 0 ].innerHTML.match( /5$/ ), "Check for appending a number" );
} );
...
QUnit.test( "XML DOM manipulation (#9960)", function( assert ) {
    ...
    var xmlDoc1 = sQuery.parseXML( "<scxml xmlns='http://www.w3.org/2005/07/scxml' version='1.0'><state x='100' y='100' initial='actions' id='provisioning'></state><state x='100' y='100' id='error'></state><state x='100' y='100' id='finished' final='true'></state></scxml>" ),
        xmlDoc2 = sQuery.parseXML( "<scxml xmlns='http://www.w3.org/2005/07/scxml' version='1.0'><state id='provisioning3'></state></scxml>" ),
        xml1 = sQuery( xmlDoc1 ),
        xml2 = sQuery( xmlDoc2 ),
        scxml1 = sQuery( "scxml", xml1 ),
        scxml2 = sQuery( "scxml", xml2 ),
    ...
    assert.deepEqual( sQuery( "state", xml2 ).get(), scxml1.find( "state" ).get(), "replaceWith" );
} );
...
QUnit.test( "append HTML5 sectioning elements (Bug #6485)", function( assert ) {
    ...
    sQuery( "#qunit-fixture" ).append( "<article style='font-size:10px'><section><aside>HTML5 elements</aside></section></article>" );

    article = sQuery( "article" );
    aside = sQuery( "aside" );
...
if ( sQuery.css ) {
    QUnit.test( "HTML5 Elements inherit styles from style rules (Bug #10501)", function( assert ) {
        ...
        sQuery( "#qunit-fixture" ).append( "<article id='article'></article>" );
        sQuery( "#article" ).append( "<section>This section should have a pink background.</section>" );

        // In IE, the missing background color will claim its value is "transparent"
        assert.notEqual( sQuery( "section" ).css( "background-color" ), "transparent", "HTML5 elements inherit styles" );
    } );
}
...
QUnit.test( "html(String) with HTML5 (Bug #6485)", function( assert ) {
    ...
    sQuery( "#qunit-fixture" ).html( "<article><section><aside>HTML5 elements</aside></section></article>" );
    assert.equal( sQuery( "#qunit-fixture" ).children().children().length, 1, "Make sure HTML5 article elements can hold children. innerHTML shortcut path" );
    assert.equal( sQuery( "#qunit-fixture" ).children().children().children().length, 1, "Make sure nested HTML5 elements can hold children." );
} );
...
QUnit.test( "html(String) tag-hyphenated elements (Bug #1987)", function( assert ) {
    ...
    sQuery.each( "thead tbody tfoot colgroup caption tr th td".split( " " ), function( i, name ) {
        var j = sQuery( "<" + name + "-d></" + name + "-d><" + name + "-d></" + name + "-d>" );
        assert.ok( j[ 0 ], "Create a tag-hyphenated element" );
        assert.ok( sQuery.nodeName( j[ 0 ], name.toUpperCase() + "-D" ), "Hyphenated node name" );
        assert.ok( sQuery.nodeName( j[ 1 ], name.toUpperCase() + "-D" ), "Hyphenated node name" );
    } );

    var j = sQuery( "<tr-multiple-hyphens><td-with-hyphen>text</td-with-hyphen></tr-multiple-hyphens>" );
    assert.ok( sQuery.nodeName( j[ 0 ], "TR-MULTIPLE-HYPHENS" ), "Tags with multiple hyphens" );
    assert.ok( sQuery.nodeName( j.children()[ 0 ], "TD-WITH-HYPHEN" ), "Tags with multiple hyphens" );
...
QUnit.test( "Tag name processing respects the HTML Standard (gh-2005)", function( assert ) {
    ...
    sQuery.each( specialChars, function( i, characters ) {
        assertSpecialCharsSupport( "html", characters );
        assertSpecialCharsSupport( "append", characters );
    } );

    sQuery.each( nameTerminatingChars, function( i, character ) {
...
QUnit.test( "html() object element #10324", function( assert ) {
    ...
    var object = sQuery( "<object id='object2'><param name='object2test' value='test'></param></object>?" ).appendTo( "#qunit-fixture" ),
        clone = object.clone();
...
QUnit.test( "append(xml)", function( assert ) {
    ...
    assert.ok( sQuery( xml1 ).append( xml2 ), "Append an xml element to another without raising an exception." );
...
QUnit.test( "appendTo(String)", function( assert ) {
    ...
    defaultText = "Try them out:";
    sQuery( "<b>buga</b>" ).appendTo( "#first" );
    assert.equal( sQuery( "#first" ).text(), defaultText + "buga", "Check if text appending works" );
    assert.equal( sQuery( "<option value='appendTest'>Append Test</option>" ).appendTo( "#select3" ).parent().find( "option:last-child" ).attr( "value" ), "appendTest", "Appending html options to select element" );

    l = sQuery( "#first" ).children().length + 2;
    sQuery( "<strong>test</strong>" );
    sQuery( "<strong>test</strong>" );
    sQuery( [ sQuery( "<strong>test</strong>" )[ 0 ], sQuery( "<strong>test</strong>" )[ 0 ] ] )
        .appendTo( "#first" );
    assert.equal( sQuery( "#first" ).children().length, l, "Make sure the elements were inserted." );
    assert.equal( sQuery( "#first" ).children().last()[ 0 ].nodeName.toLowerCase(), "strong", "Verify the last element." );
} );
...
QUnit.test( "appendTo(Element|Array<Element>)", function( assert ) {
    ...
    var expected = "This link has class=\"blog\": Simon Willison's WeblogTry them out:";
    sQuery( document.getElementById( "first" ) ).appendTo( "#sap" );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for appending of element" );

    expected = "This link has class=\"blog\": Simon Willison's WeblogTry them out:Yahoo";
    sQuery( [ document.getElementById( "first" ), document.getElementById( "yahoo" ) ] ).appendTo( "#sap" );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for appending of array of elements" );

} );

QUnit.test( "appendTo(sQuery)", function( assert ) {

    assert.expect( 10 );

    var expected, num, div;
    assert.ok( sQuery( document.createElement( "script" ) ).appendTo( "body" ).length, "Make sure a disconnected script can be appended." );

    expected = "This link has class=\"blog\": Simon Willison's WeblogYahooTry them out:";
    sQuery( "#yahoo, #first" ).appendTo( "#sap" );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for appending of sQuery object" );

    sQuery( "#select1" ).appendTo( "#foo" );
    assert.t( "Append select", "#foo select", [ "select1" ] );

    div = sQuery( "<div/>" ).on( "click", function() {
        assert.ok( true, "Running a cloned click." );
    } );
    div.appendTo( "#qunit-fixture, #moretests" );

    sQuery( "#qunit-fixture div" ).last().trigger( "click" );
    sQuery( "#moretests div" ).last().trigger( "click" );

    div = sQuery( "<div/>" ).appendTo( "#qunit-fixture, #moretests" );

    assert.equal( div.length, 2, "appendTo returns the inserted elements" );

    div.addClass( "test" );

    assert.ok( sQuery( "#qunit-fixture div" ).last().hasClass( "test" ), "appendTo element was modified after the insertion" );
    assert.ok( sQuery( "#moretests div" ).last().hasClass( "test" ), "appendTo element was modified after the insertion" );

    div = sQuery( "<div/>" );
    sQuery( "<span>a</span><b>b</b>" ).filter( "span" ).appendTo( div );

    assert.equal( div.children().length, 1, "Make sure the right number of children were inserted." );

    div = sQuery( "#moretests div" );

    num = sQuery( "#qunit-fixture div" ).length;
    div.remove().appendTo( "#qunit-fixture" );

    assert.equal( sQuery( "#qunit-fixture div" ).length, num, "Make sure all the removed divs were inserted." );
} );
...
QUnit.test( "prepend(String)", function( assert ) {
    ...
    result = sQuery( "#first" ).prepend( "<b>buga</b>" );
    assert.equal( result.text(), "buga" + expected, "Check if text prepending works" );
    assert.equal( sQuery( "#select3" ).prepend( "<option value='prependTest'>Prepend Test</option>"  ).find( "option:first-child" ).attr( "value" ), "prependTest", "Prepending html options to select element" );
} );
...
QUnit.test( "prepend(Element)", function( assert ) {
    ...
    sQuery( "#sap" ).prepend( document.getElementById( "first" ) );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of element" );
} );
...
QUnit.test( "prepend(Array<Element>)", function( assert ) {
    ...
    sQuery( "#sap" ).prepend( [ document.getElementById( "first" ), document.getElementById( "yahoo" ) ] );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of array of elements" );
} );

QUnit.test( "prepend(sQuery)", function( assert ) {

    assert.expect( 1 );

    var expected;
    expected = "YahooTry them out:This link has class=\"blog\": Simon Willison's Weblog";
    sQuery( "#sap" ).prepend( sQuery( "#yahoo, #first" ) );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of sQuery object" );
} );

QUnit.test( "prepend(Array<sQuery>)", function( assert ) {

    assert.expect( 1 );

    var expected;
    expected = "Try them out:GoogleYahooThis link has class=\"blog\": Simon Willison's Weblog";
    sQuery( "#sap" ).prepend( [ sQuery( "#first" ), sQuery( "#yahoo, #google" ) ] );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of array of sQuery objects" );
} );
...
QUnit.test( "prepend(Function) with incoming value -- String", function( assert ) {
    ...
    defaultText = "Try them out:";
    old = sQuery( "#first" ).html();
    result = sQuery( "#first" ).prepend( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return "<b>buga</b>";
    } );
    ...
    old = sQuery( "#select3" ).html();

    assert.equal( sQuery( "#select3" ).prepend( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
...
QUnit.test( "prepend(Function) with incoming value -- Element", function( assert ) {
    ...
    sQuery( "#sap" ).prepend( function( i, val ) {
        assert.equal( val, old, "Make sure the incoming value is correct." );
        return document.getElementById( "first" );
    } );
...
QUnit.test( "prepend(Function) with incoming value -- sQuery", function( assert ) {
...
QUnit.test( "prependTo(String)", function( assert ) {
    ...
    defaultText = "Try them out:";
    sQuery( "<b>buga</b>" ).prependTo( "#first" );
    assert.equal( sQuery( "#first" ).text(), "buga" + defaultText, "Check if text prepending works" );
    assert.equal( sQuery( "<option value='prependTest'>Prepend Test</option>" ).prependTo( "#select3" ).parent().find( "option:first-child" ).attr( "value" ), "prependTest", "Prepending html options to select element" );
...
QUnit.test( "prependTo(Element)", function( assert ) {
    ...
    expected = "Try them out:This link has class=\"blog\": Simon Willison's Weblog";
    sQuery( document.getElementById( "first" ) ).prependTo( "#sap" );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of element" );
} );
...
QUnit.test( "prependTo(Array<Element>)", function( assert ) {
    ...
    expected = "Try them out:YahooThis link has class=\"blog\": Simon Willison's Weblog";
    sQuery( [ document.getElementById( "first" ), document.getElementById( "yahoo" ) ] ).prependTo( "#sap" );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of array of elements" );
} );

QUnit.test( "prependTo(sQuery)", function( assert ) {
    ...
    expected = "YahooTry them out:This link has class=\"blog\": Simon Willison's Weblog";
    sQuery( "#yahoo, #first" ).prependTo( "#sap" );
    assert.equal( sQuery( "#sap" ).text(), expected, "Check for prepending of sQuery object" );
} );

QUnit.test( "prependTo(Array<sQuery>)", function( assert ) {

    assert.expect( 1 );

    sQuery( "<select id='prependSelect1'></select>" ).prependTo( "#form" );
    sQuery( "<select id='prependSelect2'><option>Test</option></select>" ).prependTo( "#form" );
...
QUnit.test( "before(String)", function( assert ) {
    ...
    sQuery( "#yahoo" ).before( manipulationBareObj( "<b>buga</b>" ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert String before" );
} );
...
QUnit.test( "before(Element)", function( assert ) {
    ...
    sQuery( "#yahoo" ).before( manipulationBareObj( document.getElementById( "first" ) ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert element before" );
} );
...
QUnit.test( "before(Array<Element>)", function( assert ) {
    ...
    sQuery( "#yahoo" ).before( manipulationBareObj( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of elements before" );
} );

QUnit.test( "before(sQuery)", function( assert ) {

    assert.expect( 1 );

    var expected;
    expected = "This is a normal link: diveintomarkTry them out:Yahoo";
    sQuery( "#yahoo" ).before( manipulationBareObj( sQuery( "#mark, #first" ) ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert sQuery before" );
} );

QUnit.test( "before(Array<sQuery>)", function( assert ) {

    assert.expect( 1 );

    var expected;
    expected = "This is a normal link: Try them out:GooglediveintomarkYahoo";
    sQuery( "#yahoo" ).before( manipulationBareObj( [ sQuery( "#first" ), sQuery( "#mark, #google" ) ] ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of sQuery objects before" );
} );
...
QUnit.test( "before(Function) -- Returns String", function( assert ) {
    ...
    expected = "This is a normal link: bugaYahoo";
    sQuery( "#yahoo" ).before( manipulationFunctionReturningObj( "<b>buga</b>" ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert String before" );
} );
...
QUnit.test( "before(Function) -- Returns Element", function( assert ) {
    ...
    expected = "This is a normal link: Try them out:Yahoo";
    sQuery( "#yahoo" ).before( manipulationFunctionReturningObj( document.getElementById( "first" ) ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert element before" );
} );
...
QUnit.test( "before(Function) -- Returns Array<Element>", function( assert ) {
    ...
    var expected;
    expected = "This is a normal link: Try them out:diveintomarkYahoo";
    sQuery( "#yahoo" ).before( manipulationFunctionReturningObj( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of elements before" );
} );

QUnit.test( "before(Function) -- Returns sQuery", function( assert ) {

    assert.expect( 1 );

    var expected;
    expected = "This is a normal link: diveintomarkTry them out:Yahoo";
    sQuery( "#yahoo" ).before( manipulationFunctionReturningObj( sQuery( "#mark, #first" ) ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert sQuery before" );
} );

QUnit.test( "before(Function) -- Returns Array<sQuery>", function( assert ) {

    assert.expect( 1 );

    var expected;
    expected = "This is a normal link: Try them out:GooglediveintomarkYahoo";
    sQuery( "#yahoo" ).before( manipulationFunctionReturningObj( [ sQuery( "#first" ), sQuery( "#mark, #google" ) ] ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of sQuery objects before" );
} );
...
QUnit.test( "before(no-op)", function( assert ) {
    ...
    var set;
    set = sQuery( "<div/>" ).before( "<span>test</span>" );
...
QUnit.test( "before and after w/ empty object (#10812)", function( assert ) {
    ...
    res = sQuery( "#notInTheDocument" ).before( "(" ).after( ")" );
...
QUnit.test( ".before() and .after() disconnected node", function( assert ) {
    ...
    assert.equal( sQuery( "<input type='checkbox'/>" ).before( "<div/>" ).length, 1, "before() on disconnected node is no-op" );
    assert.equal( sQuery( "<input type='checkbox'/>" ).after( "<div/>" ).length, 1, "after() on disconnected node is no-op" );
} );
...
QUnit.test( "insert with .before() on disconnected node last", function( assert ) {
    ...
    sQuery( "#yahoo" ).add( "<span/>" ).before( "<b>buga</b>" );
    assert.equal( sQuery( "#en" ).text(), expectedBefore, "Insert String before with disconnected node last" );
} );
...
QUnit.test( "insert with .before() on disconnected node first", function( assert ) {
    ...
    sQuery( "<span/>" ).add( "#yahoo" ).before( "<b>buga</b>" );
    assert.equal( sQuery( "#en" ).text(), expectedBefore, "Insert String before with disconnected node first" );
} );
...
QUnit.test( "insert with .before() on disconnected node last", function( assert ) {
    ...
    sQuery( "#yahoo" ).add( "<span/>" ).after( "<b>buga</b>" );
    assert.equal( sQuery( "#en" ).text(), expectedAfter, "Insert String after with disconnected node last" );
} );
...
QUnit.test( "insert with .before() on disconnected node last", function( assert ) {
    ...
    sQuery( "<span/>" ).add( "#yahoo" ).after( "<b>buga</b>" );
    assert.equal( sQuery( "#en" ).text(), expectedAfter, "Insert String after with disconnected node first" );
} );
...
QUnit.test( "insertBefore(String)", function( assert ) {
    ...
    sQuery( "<b>buga</b>" ).insertBefore( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert String before" );
} );
...
QUnit.test( "insertBefore(Element)", function( assert ) {
    ...
    sQuery( document.getElementById( "first" ) ).insertBefore( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert element before" );
} );
...
QUnit.test( "insertBefore(Array<Element>)", function( assert ) {
    ...
    sQuery( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ).insertBefore( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of elements before" );
} );

QUnit.test( "insertBefore(sQuery)", function( assert ) {

    assert.expect( 1 );

    var expected = "This is a normal link: diveintomarkTry them out:Yahoo";
    sQuery( "#mark, #first" ).insertBefore( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert sQuery before" );
} );
...
QUnit.test( ".after(String)", function( assert ) {
    ...
    sQuery( "#yahoo" ).after( "<b>buga</b>" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert String after" );
} );
...
QUnit.test( ".after(Element)", function( assert ) {
    ...
    sQuery( "#yahoo" ).after( document.getElementById( "first" ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert element after" );
} );
...
QUnit.test( ".after(Array<Element>)", function( assert ) {
    ...
    sQuery( "#yahoo" ).after( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of elements after" );
} );

QUnit.test( ".after(sQuery)", function( assert ) {

    assert.expect( 1 );

    var expected = "This is a normal link: YahooTry them out:Googlediveintomark";
    sQuery( "#yahoo" ).after( [ sQuery( "#first" ), sQuery( "#mark, #google" ) ] );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of sQuery objects after" );
} );
...
QUnit.test( ".after(Function) returns String", function( assert ) {
    ...
    sQuery( "#yahoo" ).after( val( "<b>buga</b>" ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert String after" );
} );
...
QUnit.test( ".after(Function) returns Element", function( assert ) {
    ...
    var expected = "This is a normal link: YahooTry them out:",
        val = manipulationFunctionReturningObj;
    sQuery( "#yahoo" ).after( val( document.getElementById( "first" ) ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert element after" );
} );
...
QUnit.test( ".after(Function) returns Array<Element>", function( assert ) {
    ...
    var expected = "This is a normal link: YahooTry them out:diveintomark",
        val = manipulationFunctionReturningObj;
    sQuery( "#yahoo" ).after( val( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of elements after" );
} );

QUnit.test( ".after(Function) returns sQuery", function( assert ) {

    assert.expect( 1 );

    var expected = "This is a normal link: YahooTry them out:Googlediveintomark",
        val = manipulationFunctionReturningObj;
    sQuery( "#yahoo" ).after( val( [ sQuery( "#first" ), sQuery( "#mark, #google" ) ] ) );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of sQuery objects after" );
} );
...
QUnit.test( ".after(disconnected node)", function( assert ) {
    ...
    var set = sQuery( "<div/>" ).before( "<span>test</span>" );
...
QUnit.test( "insertAfter(String)", function( assert ) {
    ...
    var expected = "This is a normal link: Yahoobuga";
    sQuery( "<b>buga</b>" ).insertAfter( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert String after" );
} );
...
QUnit.test( "insertAfter(Element)", function( assert ) {
    ...
    var expected = "This is a normal link: YahooTry them out:";
    sQuery( document.getElementById( "first" ) ).insertAfter( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert element after" );
} );
...
QUnit.test( "insertAfter(Array<Element>)", function( assert ) {
    ...
    var expected = "This is a normal link: YahooTry them out:diveintomark";
    sQuery( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ).insertAfter( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert array of elements after" );
} );

QUnit.test( "insertAfter(sQuery)", function( assert ) {

    assert.expect( 1 );

    var expected = "This is a normal link: YahoodiveintomarkTry them out:";
    sQuery( "#mark, #first" ).insertAfter( "#yahoo" );
    assert.equal( sQuery( "#en" ).text(), expected, "Insert sQuery after" );
} );
...
function testReplaceWith( val, assert ) {
    ...
    sQuery( "#yahoo" ).replaceWith( val( "<b id='replace'>buga</b>" ) );
    assert.ok( sQuery( "#replace" )[ 0 ], "Replace element with element from string" );
    assert.ok( !sQuery( "#yahoo" )[ 0 ], "Verify that original element is gone, after string" );

    sQuery( "#anchor2" ).replaceWith( val( document.getElementById( "first" ) ) );
    assert.ok( sQuery( "#first" )[ 0 ], "Replace element with element" );
    assert.ok( !sQuery( "#anchor2" )[ 0 ], "Verify that original element is gone, after element" );

    sQuery( "#qunit-fixture" ).append( "<div id='bar'><div id='baz'></div></div>" );
    sQuery( "#baz" ).replaceWith( val( "Baz" ) );
    assert.equal( sQuery( "#bar" ).text(), "Baz", "Replace element with text" );
    assert.ok( !sQuery( "#baz" )[ 0 ], "Verify that original element is gone, after element" );

    sQuery( "#bar" ).replaceWith( "<div id='yahoo'></div>", "...", "<div id='baz'></div>" );
    assert.deepEqual( sQuery( "#yahoo, #baz" ).get(), q( "yahoo", "baz" ),  "Replace element with multiple arguments (#13722)" );
    assert.strictEqual( sQuery( "#yahoo" )[ 0 ].nextSibling, sQuery( "#baz" )[ 0 ].previousSibling, "Argument order preserved" );
    assert.deepEqual( sQuery( "#bar" ).get(), [], "Verify that original element is gone, after multiple arguments" );

    sQuery( "#google" ).replaceWith( val( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ) );
    assert.deepEqual( sQuery( "#mark, #first" ).get(), q( "first", "mark" ),  "Replace element with array of elements" );
    assert.ok( !sQuery( "#google" )[ 0 ], "Verify that original element is gone, after array of elements" );

    sQuery( "#groups" ).replaceWith( val( sQuery( "#mark, #first" ) ) );
    assert.deepEqual( sQuery( "#mark, #first" ).get(), q( "first", "mark" ),  "Replace element with sQuery collection" );
    assert.ok( !sQuery( "#groups" )[ 0 ], "Verify that original element is gone, after sQuery collection" );

    sQuery( "#mark, #first" ).replaceWith( val( "<span class='replacement'></span><span class='replacement'></span>" ) );
    assert.equal( sQuery( "#qunit-fixture .replacement" ).length, 4, "Replace multiple elements (#12449)" );
    assert.deepEqual( sQuery( "#mark, #first" ).get(), [], "Verify that original elements are gone, after replace multiple" );

    tmp = sQuery( "<b>content</b>" )[ 0 ];
    sQuery( "#anchor1" ).contents().replaceWith( val( tmp ) );
    assert.deepEqual( sQuery( "#anchor1" ).contents().get(), [ tmp ], "Replace text node with element" );

    tmp = sQuery( "<div/>" ).appendTo( "#qunit-fixture" ).on( "click", function() {
        assert.ok( true, "Newly bound click run." );
    } );
    y = sQuery( "<div/>" ).appendTo( "#qunit-fixture" ).on( "click", function() {
    ...
    set = sQuery( "<div/>" ).replaceWith( val( "<span>test</span>" ) );
    ...
    child = sQuery( "#qunit-fixture" ).children().first();
    $div = sQuery( "<div class='pathological'/>" ).insertBefore( child );
    $div.replaceWith( $div );
    assert.deepEqual( sQuery( ".pathological", "#qunit-fixture" ).get(), $div.get(),
        "Self-replacement" );
    $div.replaceWith( child );
    assert.deepEqual( sQuery( "#qunit-fixture" ).children().first().get(), child.get(),
        "Replacement with following sibling (#13810)" );
    assert.deepEqual( sQuery( ".pathological", "#qunit-fixture" ).get(), [],
        "Replacement with following sibling (context removed)" );

    nonExistent = sQuery( "#does-not-exist" ).replaceWith( val( "<b>should not throw an error</b>" ) );
    assert.equal( nonExistent.length, 0, "Length of non existent element." );

    $div = sQuery( "<div class='replacewith'></div>" ).appendTo( "#qunit-fixture" );
    $div.replaceWith( val( "<div class='replacewith'></div><script>" +
        "equal( sQuery('.replacewith').length, 1, 'Check number of elements in page.' );" +
        "</script>" ) );

    sQuery( "#qunit-fixture" ).append( "<div id='replaceWith'></div>" );
    assert.equal( sQuery( "#qunit-fixture" ).find( "div[id=replaceWith]" ).length, 1, "Make sure only one div exists." );
    sQuery( "#replaceWith" ).replaceWith( val( "<div id='replaceWith'></div>" ) );
    assert.equal( sQuery( "#qunit-fixture" ).find( "div[id=replaceWith]" ).length, 1, "Make sure only one div exists after replacement." );
    sQuery( "#replaceWith" ).replaceWith( val( "<div id='replaceWith'></div>" ) );
    assert.equal( sQuery( "#qunit-fixture" ).find( "div[id=replaceWith]" ).length, 1, "Make sure only one div exists after subsequent replacement." );

    return expected;
}

QUnit.test( "replaceWith(String|Element|Array<Element>|sQuery)", function( assert ) {
...
QUnit.test( "replaceWith(string) for more than one element", function( assert ) {
    ...
    assert.equal( sQuery( "#foo p" ).length, 3, "ensuring that test data has not changed" );

    sQuery( "#foo p" ).replaceWith( "<span>bar</span>" );
    assert.equal( sQuery( "#foo span" ).length, 3, "verify that all the three original element have been replaced" );
    assert.equal( sQuery( "#foo p" ).length, 0, "verify that all the three original element have been replaced" );
} );
...
QUnit.test( "Empty replaceWith (trac-13401; trac-13596; gh-2204)", function( assert ) {
    ...
    var $el = sQuery( "<div/><div/>" ).html( "<p>0</p>" ),
        expectedHTML = $el.html(),
        tests = {
            "empty string": "",
            "empty array": [],
            "array of empty string": [ "" ],
            "empty collection": sQuery( "#nonexistent" ),

            // in case of sQuery(...).replaceWith();
            "undefined": undefined
        };

    sQuery.each( tests, function( label, input ) {
...
QUnit.test( "replaceAll(String)", function( assert ) {
    ...
    sQuery( "<b id='replace'>buga</b>" ).replaceAll( "#yahoo" );
    assert.ok( sQuery( "#replace" )[ 0 ], "Replace element with string" );
    assert.ok( !sQuery( "#yahoo" )[ 0 ], "Verify that original element is gone, after string" );
} );
...
QUnit.test( "replaceAll(Element)", function( assert ) {
    ...
    sQuery( document.getElementById( "first" ) ).replaceAll( "#yahoo" );
    assert.ok( sQuery( "#first" )[ 0 ], "Replace element with element" );
    assert.ok( !sQuery( "#yahoo" )[ 0 ], "Verify that original element is gone, after element" );
} );
...
QUnit.test( "replaceAll(Array<Element>)", function( assert ) {
    ...
    sQuery( [ document.getElementById( "first" ), document.getElementById( "mark" ) ] ).replaceAll( "#yahoo" );
    assert.ok( sQuery( "#first" )[ 0 ], "Replace element with array of elements" );
    assert.ok( sQuery( "#mark" )[ 0 ], "Replace element with array of elements" );
    assert.ok( !sQuery( "#yahoo" )[ 0 ], "Verify that original element is gone, after array of elements" );
} );

QUnit.test( "replaceAll(sQuery)", function( assert ) {

    assert.expect( 3 );

    sQuery( "#mark, #first" ).replaceAll( "#yahoo" );
    assert.ok( sQuery( "#first" )[ 0 ], "Replace element with set of elements" );
    assert.ok( sQuery( "#mark" )[ 0 ], "Replace element with set of elements" );
    assert.ok( !sQuery( "#yahoo" )[ 0 ], "Verify that original element is gone, after set of elements" );
} );

QUnit.test( "sQuery.clone() (#8017)", function( assert ) {

    assert.expect( 2 );

    assert.ok( sQuery.clone && sQuery.isFunction( sQuery.clone ), "sQuery.clone() utility exists and is a function." );

    var main = sQuery( "#qunit-fixture" )[ 0 ],
        clone = sQuery.clone( main );
...
QUnit.test( "append to multiple elements (#8070)", function( assert ) {
    ...
    var selects = sQuery( "<select class='test8070'></select><select class='test8070'></select>" ).appendTo( "#qunit-fixture" );
...
QUnit.test( "table manipulation", function( assert ) {
    ...
    var table = sQuery( "<table style='font-size:16px'></table>" ).appendTo( "#qunit-fixture" ).empty(),
...
QUnit.test( "clone()", function( assert ) {
    ...
    assert.equal( sQuery( "#en" ).text(), "This is a normal link: Yahoo", "Assert text for #en" );
    assert.equal( sQuery( "#first" ).append( sQuery( "#yahoo" ).clone() ).text(), "Try them out:Yahoo", "Check for clone" );
    assert.equal( sQuery( "#en" ).text(), "This is a normal link: Yahoo", "Reassert text for #en" );

    sQuery.each( "table thead tbody tfoot tr td div button ul ol li select option textarea iframe".split( " " ), function( i, nodeName ) {
        assert.equal( sQuery( "<" + nodeName + "/>" ).clone()[ 0 ].nodeName.toLowerCase(), nodeName, "Clone a " + nodeName );
    } );
    assert.equal( sQuery( "<input type='checkbox' />" ).clone()[ 0 ].nodeName.toLowerCase(), "input", "Clone a <input type='checkbox' />" );

    // Check cloning non-elements
    assert.equal( sQuery( "#nonnodes" ).contents().clone().length, 3, "Check node,textnode,comment clone works (some browsers delete comments on clone)" );

    // Verify that clones of clones can keep event listeners
    div = sQuery( "<div><ul><li>test</li></ul></div>" ).on( "click", function() {
    ...
    div = sQuery( "<div/>" ).append( [ document.createElement( "table" ), document.createElement( "table" ) ] );
    ...
    div = sQuery( "<div/>" ).html( "<embed height='355' width='425' src='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'></embed>" );
    ...
    div = sQuery( "<div/>" ).html( "<object height='355' width='425' classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000'>  <param name='movie' value='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='wmode' value='transparent'> </object>" );
    ...
    sQuery.each( [ "height", "width", "classid" ], function( i, attr ) {
    ...
    div = sQuery( "<div/>" ).html( "<object height='355' width='425' type='application/x-shockwave-flash' data='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='movie' value='http://www.youtube.com/v/3KANI2dpXLw&amp;hl=en'>  <param name='wmode' value='transparent'> </object>" );
    ...
    assert.equal( sQuery( form ).clone().children().length, 1, "Make sure we just get the form back." );

    body = sQuery( "body" ).clone();
...
QUnit.test( "clone(script type=non-javascript) (#11359)", function( assert ) {
    ...
    var src = sQuery( "<script type='text/filler'>Lorem ipsum dolor sit amet</script><q><script type='text/filler'>consectetur adipiscing elit</script></q>" ),
        dest = src.clone();
...
QUnit.test( "clone(form element) (Bug #3879, #6655)", function( assert ) {
    ...
    element = sQuery( "<select><option>Foo</option><option value='selected' selected>Bar</option></select>" );
    ...
    element = sQuery( "<input type='checkbox' value='foo'>" ).attr( "checked", "checked" );
    clone = element.clone();
...
QUnit.test( "clone(multiple selected options) (Bug #8129)", function( assert ) {
    ...
    var element = sQuery( "<select><option>Foo</option><option selected>Bar</option><option selected>Baz</option></select>" );
...
QUnit.test( "clone() on XML nodes", function( assert ) {
    ...
    var xml = createDashboardXML(),
        root = sQuery( xml.documentElement ).clone(),
        origTab = sQuery( "tab", xml ).eq( 0 ),
        cloneTab = sQuery( "tab", root ).eq( 0 );
...
QUnit.test( "clone() on local XML nodes with html5 nodename", function( assert ) {
    ...
    var $xmlDoc = sQuery( sQuery.parseXML( "<root><meter /></root>" ) ),
...
QUnit.test( "html(undefined)", function( assert ) {
    ...
    assert.equal( sQuery( "#foo" ).html( "<i>test</i>" ).html( undefined ).html().toLowerCase(), "<i>test</i>", ".html(undefined) is chainable (#5571)" );
} );
...
QUnit.test( "html() on empty set", function( assert ) {
    ...
    assert.strictEqual( sQuery().html(), undefined, ".html() returns undefined for empty sets (#11962)" );
} );

function childNodeNames( node ) {
    return sQuery.map( node.childNodes, function( child ) {
...
function testHtml( valueObj, assert ) {
    ...
    actual = []; expected = [];
    tmp = sQuery( "<map/>" ).html( valueObj( "<area alt='area'/>" ) ).each( function() {
        expected.push( "AREA" );
    ...
    actual = []; expected = []; tmp = {};
    sQuery( "#nonnodes" ).contents().html( valueObj( "<b>bold</b>" ) ).each( function() {
        var html = sQuery( this ).html();
    ...
    sQuery( "#qunit-fixture select" ).html( valueObj( "<option>O1</option><option selected='selected'>O2</option><option>O3</option>" ) );
    assert.equal( sQuery( "#qunit-fixture select" ).val(), "O2", "Selected option correct" );
    ...
    sQuery.scriptorder = 0;
    fixture.html( valueObj( [
        "<script>",
        "equal( sQuery('#scriptorder').length, 1,'Execute after html' );",
        "equal( sQuery.scriptorder++, 0, 'Script is executed in order' );",
        "</script>",
        "<span id='scriptorder'><script>equal( sQuery.scriptorder++, 1, 'Script (nested) is executed in order');</script></span>",
        "<script>equal( sQuery.scriptorder++, 2, 'Script (unnested) is executed in order' );</script>"
    ].join( "" ) ) );
...
QUnit.test( "html(Function) with incoming value -- sQuery.contents()", function( assert ) {
...
QUnit.test( "clone()/html() don't expose sQuery/Sizzle expandos (#12858)", function( assert ) {

    assert.expect( 2 );

    var $content = sQuery( "<div><b><i>text</i></b></div>" ).appendTo( "#qunit-fixture" ),
        expected = /^<b><i>text<\/i><\/b>$/i;

    // Attach sQuery and Sizzle data (the latter with a non-qSA nth-child)
    try {
...
QUnit.test( "remove() no filters", function( assert ) {
    ...
    var first = sQuery( "#ap" ).children().first();

    first.data( "foo", "bar" );

    sQuery( "#ap" ).children().remove();
    assert.ok( sQuery( "#ap" ).text().length > 10, "Check text is not removed" );
    assert.equal( sQuery( "#ap" ).children().length, 0, "Check remove" );
} );
...
QUnit.test( "remove() with filters", function( assert ) {
    ...
    var markup, div;
    sQuery( "#ap" ).children().remove( "a" );
    assert.ok( sQuery( "#ap" ).text().length > 10, "Check text is not removed" );
    assert.equal( sQuery( "#ap" ).children().length, 1, "Check filtered remove" );

    sQuery( "#ap" ).children().remove( "a, code" );
    assert.equal( sQuery( "#ap" ).children().length, 0, "Check multi-filtered remove" );
    ...
    // using contents will get comments regular, text, and comment nodes
    // Handle the case where no comment is in the document
    assert.ok( sQuery( "#nonnodes" ).contents().length >= 2, "Check node,textnode,comment remove works" );
    sQuery( "#nonnodes" ).contents().remove();
    assert.equal( sQuery( "#nonnodes" ).contents().length, 0, "Check node,textnode,comment remove works" );
} );
...
QUnit.test( "remove() event cleaning ", function( assert ) {
    ...
    count = 0;
    first = sQuery( "#ap" ).children().first();
...
QUnit.test( "remove() in document order #13779", function( assert ) {
    ...
    var last,
        cleanData = sQuery.cleanData;

    sQuery.cleanData = function( nodes ) {
        last = sQuery.text( nodes[ 0 ] );
        cleanData.call( this, nodes );
    };

    sQuery( "#qunit-fixture" ).append(
        sQuery.parseHTML(
...
QUnit.test( "detach() with filters", function( assert ) {
    ...
    var markup, div;
    sQuery( "#ap" ).children().detach( "a" );
    assert.ok( sQuery( "#ap" ).text().length > 10, "Check text is not removed" );
    assert.equal( sQuery( "#ap" ).children().length, 1, "Check filtered remove" );

    sQuery( "#ap" ).children().detach( "a, code" );
    assert.equal( sQuery( "#ap" ).children().length, 0, "Check multi-filtered remove" );
    ...
    // Handle the case where no comment is in the document
    assert.ok( sQuery( "#nonnodes" ).contents().length >= 2, "Check node,textnode,comment remove works" );
    sQuery( "#nonnodes" ).contents().detach();
    assert.equal( sQuery( "#nonnodes" ).contents().length, 0, "Check node,textnode,comment remove works" );
} );
...
QUnit.test( "empty()", function( assert ) {
    ...
    assert.equal( sQuery( "#ap" ).children().empty().text().length, 0, "Check text is removed" );
    assert.equal( sQuery( "#ap" ).children().length, 4, "Check elements are not removed" );

    // using contents will get comments regular, text, and comment nodes
    var j = sQuery( "#nonnodes" ).contents();
    j.empty();
    assert.equal( j.html(), "", "Check node,textnode,comment empty works" );
} );

QUnit.test( "sQuery.cleanData", function( assert ) {
    ...
    function getDiv() {
        var div = sQuery( "<div class='outer'><div class='inner'></div></div>" ).on( "click", function() {
...
QUnit.test( "sQuery.cleanData eliminates all private data (gh-2127)", function( assert ) {
    assert.expect( 3 );

    var div = sQuery( "<div/>" ).appendTo( "#qunit-fixture" );

    sQuery._data( div[ 0 ], "gh-2127", "testing" );

    assert.ok( !sQuery.isEmptyObject( sQuery._data( div[ 0 ] ) ),  "Ensure some private data exists" );

    div.remove();

    assert.ok( !sQuery.hasData( div[ 0 ] ), "Removed element hasData should return false" );

    assert.ok( sQuery.isEmptyObject( sQuery._data( div[ 0 ] ) ),
        "Private data is empty after node is removed" );

    div.remove();
} );

QUnit.test( "sQuery.cleanData eliminates all public data", function( assert ) {
    assert.expect( 3 );

    var key,
        div = sQuery( "<div/>" );
    div.data( "some", "data" );
    assert.ok( !sQuery.isEmptyObject( sQuery.data( div[ 0 ] ) ),  "Ensure some public data exists" );
...
QUnit.test( "domManip plain-text caching (trac-6779)", function( assert ) {
    ...
    var i,
        $f = sQuery( "<div />" ).appendTo( "#qunit-fixture" ),
...
//     function( assert, sQuery, window, document, test ) {
//         assert.expect( 1 );
...
//     function( assert, framesQuery, frameWindow, frameDocument ) {
//         assert.expect( 2 );
//         sQuery( frameDocument.body ).append( "<script>window.scriptTest = true;<\x2fscript>" );
...
QUnit.test( "sQuery.clone - no exceptions for object elements #9587", function( assert ) {

    assert.expect( 1 );

    try {
        sQuery( "#no-clone-exception" ).clone();
...
QUnit.test( "Cloned, detached HTML5 elems (#10667,10670)", function( assert ) {
    ...
    var $clone,
        $section = sQuery( "<section>" ).appendTo( "#qunit-fixture" );
...
QUnit.test( "Ensure oldIE creates a new set on appendTo (#8894)", function( assert ) {
    ...
    assert.strictEqual( sQuery( "<div/>" ).clone().addClass( "test" ).appendTo( "<div/>" ).end().end().hasClass( "test" ), false, "Check sQuery.fn.appendTo after sQuery.clone" );
    assert.strictEqual( sQuery( "<div/>" ).find( "p" ).end().addClass( "test" ).appendTo( "<div/>" ).end().end().hasClass( "test" ), false, "Check sQuery.fn.appendTo after sQuery.fn.find" );
    assert.strictEqual( sQuery( "<div/>" ).text( "test" ).addClass( "test" ).appendTo( "<div/>" ).end().end().hasClass( "test" ), false, "Check sQuery.fn.appendTo after sQuery.fn.text" );
    assert.strictEqual( sQuery( "<bdi/>" ).clone().addClass( "test" ).appendTo( "<div/>" ).end().end().hasClass( "test" ), false, "Check sQuery.fn.appendTo after clone html5 element" );
    assert.strictEqual( sQuery( "<p/>" ).appendTo( "<div/>" ).end().length, sQuery( "<p>test</p>" ).appendTo( "<div/>" ).end().length, "Elements created with createElement and with createDocumentFragment should be treated alike" );
} );
...
QUnit.asyncTest( "html() - script exceptions bubble (#11743)", 2, function( assert ) {
    ...
    window.onerror = function() {
        ...
        if ( sQuery.ajax ) {
            ...
            sQuery( "#qunit-fixture" ).html( "<script src='data/badcall.js'></script>" );
            assert.ok( true, "Exception ignored" );
        } else {
            assert.ok( true, "No sQuery.ajax" );
            assert.ok( true, "No sQuery.ajax" );
        }
    };

    sQuery( "#qunit-fixture" ).html( "<script>undefined();</script>" );
} );
...
QUnit.test( "checked state is cloned with clone()", function( assert ) {
    ...
    var elem = sQuery.parseHTML( "<input type='checkbox' checked='checked'/>" )[ 0 ];
    elem.checked = false;
    assert.equal( sQuery( elem ).clone().attr( "id", "clone" )[ 0 ].checked, false, "Checked false state correctly cloned" );

    elem = sQuery.parseHTML( "<input type='checkbox'/>" )[ 0 ];
    elem.checked = true;
    assert.equal( sQuery( elem ).clone().attr( "id", "clone" )[ 0 ].checked, true, "Checked true state correctly cloned" );
} );

QUnit.test( "manipulate mixed sQuery and text (#12384, #12346)", function( assert ) {

    assert.expect( 2 );

    var div = sQuery( "<div>a</div>" ).append( "&nbsp;", sQuery( "<span>b</span>" ), "&nbsp;", sQuery( "<span>c</span>" ) ),
        nbsp = String.fromCharCode( 160 );

    assert.equal( div.text(), "a" + nbsp + "b" + nbsp + "c", "Appending mixed sQuery with text nodes" );

    div = sQuery( "<div><div></div></div>" )
        .find( "div" )
        .after( "<p>a</p>", "<p>b</p>" )
        .parent();
...
QUnit.test( "script evaluation (#11795)", function( assert ) {
    ...
    var scriptsIn, scriptsOut,
        fixture = sQuery( "#qunit-fixture" ).empty(),
        objGlobal = ( function() {
            return this;
        } )(),
        isOk = objGlobal.ok,
    ...
    scriptsIn.appendTo( sQuery( "<div class='detached'/>" ) );
    objGlobal.ok = isOk;
    ...
    if ( sQuery.ajax ) {
        ...
        sQuery( "#qunit-fixture" ).append( "<script src='" + url( "data/testbar.php" ) + "'/>" );
...
QUnit.test( "sQuery._evalUrl (#12838)", function( assert ) {

    assert.expect( 5 );

    var message, expectedArgument,
        ajax = sQuery.ajax,
        evalUrl = sQuery._evalUrl;

    message = "sQuery.ajax implementation";
    expectedArgument = 1;
    sQuery.ajax = function( input ) {
        assert.equal( ( input.url || input ).slice( -1 ), expectedArgument, message );
        expectedArgument++;
    };
    sQuery( "#qunit-fixture" ).append( "<script src='1'/><script src='2'/>" );
    assert.equal( expectedArgument, 3, "synchronous execution" );

    message = "custom implementation";
    expectedArgument = 3;
    sQuery._evalUrl = sQuery.ajax;
    sQuery.ajax = function( options ) {
        assert.strictEqual( options, {}, "Unexpected call to sQuery.ajax" );
    };
    sQuery( "#qunit-fixture" ).append( "<script src='3'/><script src='4'/>" );

    sQuery.ajax = ajax;
    sQuery._evalUrl = evalUrl;
} );

QUnit.test( "sQuery.htmlPrefilter (gh-1747)", function( assert ) {
    ...
    var expectedArgument,
        ...
        htmlPrefilter = sQuery.htmlPrefilter,
        fixture = sQuery( "<div/>" ).appendTo( "#qunit-fixture" ),
        poison = "<script>sQuery.htmlPrefilter.assert.ok( false, 'script not executed' );</script>";

    sQuery.htmlPrefilter = function( html ) {
        invocations++;
    ...
    sQuery.htmlPrefilter.assert = assert;
    ...
    setTimeout( function() {
        sQuery.htmlPrefilter = htmlPrefilter;
        done();
    }, 100 );
} );
...
QUnit.test( "insertAfter, insertBefore, etc do not work when destination is original element. Element is removed (#4087)", function( assert ) {
    ...
    sQuery.each( [
        ...
        sQuery( "#test4087-complex div" )[ name ]( "#test4087-complex li:last-child div:last-child" );
        assert.equal( sQuery( "#test4087-complex li:last-child div" ).length, name === "replaceAll" ? 1 : 2, name + " a node to itself, complex case." );

        // simple case
        sQuery( ".test4087-1" )[ name ]( ".test4087-1" );
        assert.equal( sQuery( ".test4087-1" ).length, 1, name + " a node to itself, simple case." );

        // clean for next test
        sQuery( "#test4087-complex" ).remove();
        sQuery( "#test4087-simple" ).remove();
        sQuery( "#test4087-multiple" ).remove();
    } );
} );
...
QUnit.test( "Index for function argument should be received (#13094)", function( assert ) {
    ...
    sQuery( "<div/><div/>" ).before( function( index ) {
        assert.equal( index, i++, "Index should be correct" );
    } );

} );

QUnit.test( "Make sure sQuery.fn.remove can work on elements in documentFragment", function( assert ) {
    assert.expect( 1 );
...
QUnit.test( "Make sure specific elements with content created correctly (#13232)", function( assert ) {
    ...
    sQuery.each( elems, function( name, value ) {
        var html = "<" + name + ">" + value + "</" + name + ">";
        assert.ok( sQuery.nodeName( sQuery.parseHTML( "<" + name + ">" + value + "</" + name + ">" )[ 0 ], name ), name + " is created correctly" );

        results.push( name );
        args.push( html );
    } );

    sQuery.fn.append.apply( sQuery( "<div/>" ), args ).children().each( function( i ) {
        assert.ok( sQuery.nodeName( this, results[ i ] ) );
    } );
} );
...
QUnit.test( "Validate creation of multiple quantities of certain elements (#13818)", function( assert ) {
    ...
    sQuery.each( tags, function( index, tag ) {
        sQuery( "<" + tag + "/><" + tag + "/>" ).each( function() {
            assert.ok( sQuery.nodeName( this, tag ), tag + " empty elements created correctly" );
        } );

        sQuery( "<" + this + "></" + tag + "><" + tag + "></" + tag + ">" ).each( function() {
            assert.ok( sQuery.nodeName( this, tag ), tag + " elements with closing tag created correctly" );
        } );
    } );
} );
...
QUnit.test( "Make sure tr element will be appended to tbody element of table when present", function( assert ) {
    ...
    sQuery( table ).append( "<tr><td>test</td></tr>" );
...
QUnit.test( "Make sure tr elements will be appended to tbody element of table when present", function( assert ) {
    ...
    sQuery( table ).append( "<tr><td>1</td></tr><tr><td>2</td></tr>" );
...
QUnit.test( "Make sure col element is appended correctly", function( assert ) {
    ...
    var table = sQuery( "<table cellpadding='0'><tr><td>test</td></tr></table>" );

    sQuery( table ).appendTo( "#qunit-fixture" );

    sQuery( "<col width='150'/>" ).prependTo( table );
...
QUnit.test( "Insert script with data-URI (gh-1887)", 1, function( assert ) {
    ...
    sQuery( fixture ).append( "<script src=\"data:text/javascript,testFoo = 'foo';\"></script>" );
